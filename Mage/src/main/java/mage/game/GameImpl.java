...
public abstract class GameImpl implements Game {
    ...
    public boolean mutatePermanent(Card card, UUID permanentId, Spell source) {
        ...
        final Permanent newPermanent;
        if (source.isCopy()) {
            EmptyToken token = new EmptyToken();
            CardUtil.copyTo(token).from(card, this, source);
            newPermanent = new PermanentToken(token, card.getOwnerId(), token.getOriginalExpansionSetCode(), this);
        } else {
            newPermanent = new PermanentCard(card, card.getOwnerId(), this);
        }
