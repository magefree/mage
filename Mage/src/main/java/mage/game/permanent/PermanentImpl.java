...
public abstract class PermanentImpl extends CardImpl implements Permanent {
    ...
    public boolean cantBeAttachedBy(MageObject attachment, Ability source, Game game, boolean silentMode) {
        ...
        canAttach = (attachment.hasSubtype(SubType.AURA, game)
                && attachmentPermanent != null
                && attachmentPermanent.getSpellAbility() != null
                && !attachmentPermanent.getSpellAbility().getTargets().isEmpty()
                && attachmentPermanent.getSpellAbility().getTargets().get(0).canTarget(attachmentPermanent.getControllerId(), this.getId(), source, game)
        );
