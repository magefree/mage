...
//        this.userData = player.getUserData();
    ...
    private Cards getToDiscard(int minAmount, int maxAmount, Ability source, Game game) {
        ...
        choose(Outcome.Discard, target, source != null ? source.getSourceId() : null, source, game);
...
//        this.left = true;
    ...
    public boolean seekCard(FilterCard filter, Ability source, Game game) {
        Set<Card> cards = this.getLibrary()
                .getCards(game)
                .stream()
                .filter(card -> filter.match(card, source.getSourceId(), getId(), source, game))
    ...
    private void addCostTargetOptions(List<Ability> options, Ability option, int targetNum, Game game) {
        for (UUID targetId : option.getCosts().getTargets().get(targetNum).possibleTargets(option.getSourceId(), playerId, option, game)) {
            Ability newOption = option.copy();
    ...
    public boolean canPaySacrificeCost(Permanent permanent, Ability source, UUID controllerId, Game game) {
        return sacrificeCostFilter == null || !sacrificeCostFilter.match(permanent, source.getSourceId(), controllerId, source, game);
    }
