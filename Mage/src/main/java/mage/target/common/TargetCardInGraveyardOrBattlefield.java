...
public class TargetCardInGraveyardOrBattlefield extends TargetCard {
        ...
        if (!super.canChoose(sourceId, sourceControllerId, source, game)) {
            ...
            for (Permanent permanent : game.getBattlefield().getActivePermanents(filterBattlefield, sourceControllerId, game)) {
                if ((notTarget || permanent.canBeTargetedBy(targetSource, sourceControllerId, game))
                        && filterBattlefield.match(permanent, sourceId, sourceControllerId, source, game)) {
                    return true;
                }
            }
            return false;
    ...
    public boolean canTarget(UUID playerId, UUID id, Ability source, Game game) {
        if (!super.canTarget(playerId, id, source, game)) { // in graveyard first
            Permanent permanent = game.getPermanent(id);
            if (permanent != null) {
                return filterBattlefield.match(permanent, source != null ? source.getSourceId() : null, playerId, source, game);
            }
        }
        return true;
    }
    ...
    public Set<UUID> possibleTargets(UUID sourceControllerId, Game game) {
        ...
        for (Permanent permanent : game.getBattlefield().getActivePermanents(filterBattlefield, sourceControllerId, game)) {
            if (filterBattlefield.match(permanent, null, sourceControllerId, null, game)) {
    ...
    public Set<UUID> possibleTargets(UUID sourceId, UUID sourceControllerId, Ability source, Game game) {
        Set<UUID> possibleTargets = super.possibleTargets(sourceId, sourceControllerId, source, game); // in graveyard first
        MageObject targetSource = game.getObject(sourceId);
        for (Permanent permanent : game.getBattlefield().getActivePermanents(filterBattlefield, sourceControllerId, sourceId, source, game)) {
            if ((notTarget || permanent.canBeTargetedBy(targetSource, sourceControllerId, game)) && filterBattlefield.match(permanent, sourceId, sourceControllerId, source, game)) {
                possibleTargets.add(permanent.getId());
            }
        }
        return possibleTargets;
