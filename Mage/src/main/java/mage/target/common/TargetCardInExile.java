    ...
    public boolean canChoose(UUID sourceId, UUID sourceControllerId, Ability source, Game game) {
        if (allExileZones) {
            ...
            for(ExileZone exileZone : game.getExile().getExileZones()) {
                numberTargets += exileZone.count(filter, sourceId, sourceControllerId, source, game);
            ...
            ExileZone exileZone = game.getExile().getExileZone(zoneId);
            if (exileZone != null) {
                if (exileZone.count(filter, sourceId, sourceControllerId, source, game) >= this.minNumberOfTargets) {
                    return true;
                }
            }
        }
