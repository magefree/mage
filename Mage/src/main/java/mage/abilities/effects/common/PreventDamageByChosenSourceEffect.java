...
public class PreventDamageByChosenSourceEffect extends PreventionEffectImpl {
    ...
    public PreventDamageByChosenSourceEffect(FilterSource filter) {
        this(filter, false);
    }

    public PreventDamageByChosenSourceEffect(FilterSource filter, boolean onlyCombat) {
        super(Duration.EndOfTurn, Integer.MAX_VALUE, onlyCombat);
        if (!filter.getMessage().endsWith("source")) {
            filter.setMessage(filter.getMessage() + " source");
        }
        this.target = new TargetSource(filter);
        staticText = "Prevent all" + (onlyCombat ? " combat" : "")
                + " damage " + filter.getMessage() + " of your choice would deal this turn";
    }
