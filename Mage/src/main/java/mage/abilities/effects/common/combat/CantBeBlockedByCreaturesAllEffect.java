...
public class CantBeBlockedByCreaturesAllEffect extends RestrictionEffect {
    ...
    public boolean applies(Permanent permanent, Ability source, Game game) {
        return filterCreatures.match(permanent, source.getSourceId(), source.getControllerId(), source, game);
    }

    @Override
    public boolean canBeBlocked(Permanent attacker, Permanent blocker, Ability source, Game game, boolean canUseChooseDialogs) {
        return !filterBlockedBy.match(blocker, source.getSourceId(), source.getControllerId(), source, game);
    }
