...
public class SwitchPowerToughnessAllEffect extends ContinuousEffectImpl {
    ...
    public void init(Ability source, Game game) {
        ...
        if (this.affectedObjectsSet) {
            ...
            if (controller != null) {
                for (Permanent perm :game.getState().getBattlefield().getActivePermanents(filter, source.getControllerId(), source.getSourceId(), source, game)) {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (controller != null) {
            if (this.affectedObjectsSet) {
                ...
                for (Permanent creature :game.getState().getBattlefield().getActivePermanents(filter, source.getControllerId(), source.getSourceId(), source, game)) {
                    int power = creature.getPower().getValue();
                    creature.getPower().setValue(creature.getToughness().getValue());
                    creature.getToughness().setValue(power);                    
                }
