...
public enum UrzaTerrainValue implements DynamicValue {
    ...
    public int calculate(Game game, Ability sourceAbility, Effect effect) {
        if (subType != SubType.MINE && game.getBattlefield().count(
                mineFilter, sourceAbility.getSourceId(), sourceAbility.getControllerId(), sourceAbility, game
        ) < 1) {
            return 1;
        }
        if (subType != SubType.TOWER && game.getBattlefield().count(
                towerFilter, sourceAbility.getSourceId(), sourceAbility.getControllerId(), sourceAbility, game
        ) < 1) {
            return 1;
        }
        if (subType != SubType.POWER_PLANT && game.getBattlefield().count(
                powerPlantFilter, sourceAbility.getSourceId(), sourceAbility.getControllerId(), sourceAbility, game
        ) < 1) {
            return 1;
        }
        return value;
