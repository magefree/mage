...
public class ProtectionAbility extends StaticAbility {
    ...
    public boolean canTarget(MageObject source, Game game) {
        ...
        if (filter instanceof FilterCard) {
            if (source instanceof Permanent) {
                return !((FilterCard) filter).match((Card) source, getSourceId(), ((Permanent) source).getControllerId(), this, game);
            } else if (source instanceof Card) {
                return !((FilterCard) filter).match((Card) source, getSourceId(), ((Card) source).getOwnerId(), this, game);
            }
            return true;
        }
        ...
        if (filter instanceof FilterPlayer) {
            ...
            return !((FilterPlayer) filter).match(player, getSourceId(), this.getControllerId(), this, game);
        }
