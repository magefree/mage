...
move("../Mage.Server.Console/target/mage-server-console.zip", "server-console.zip");
...
$f->extract( to => "server/");
$f = Archive::Extract->new( archive => "server-console.zip");
$f->extract( to => "server-console/");
unlink("client.zip");
unlink("server.zip");
unlink("server-console.zip");

my $zip = Archive::Zip->new();;
$zip->addTree("client", "mage-client");
$zip->addTree("server", "mage-server");
$zip->addTree("server-console", "mage-server-console");
