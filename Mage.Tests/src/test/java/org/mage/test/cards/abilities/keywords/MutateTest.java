...
public class MutateTest extends CardTestPlayerBase {
    ...
    private static final String MORTUS_STRIDER = "Mortus Strider"; // {1}{U}{B} - 1/1 - When ~ dies, return it to its owner's hand.
    ...
    private static final String BLITZ_AUTOMATON = "Blitz Automaton"; // {7} - 6/4 - Haste; Prototype {2}{R} - 3/2
    ...
    private static final String REPTILIAN_REFLECTION = "Reptilian Reflection"; // {2}{R} - Whenever you cycle a card, you may have this enchantment become a 5/4 Dinosaur creature with trample and haste in addition to its other types until end of turn.
    private static final String INTANGIBLE_VIRTUE = "Intangible Virtue"; // {1}{W} - Creature tokens you control get +1/+1 and have vigilance
    private static final String FUNGAL_FORTITUDE = "Fungal Fortitude"; // {1}{B} - Aura - Flash - Enchanted creature gets +2/+0. When enchanted creature dies, return it to the battlefield tapped under its owner’s control.

    // Artifact
    private static final String SOL_RING = "Sol Ring"; // You know what it does....
    private static final String SHORT_SWORD = "Short Sword"; // {1} - Equip - Equip {1} - Equipped gets +1/+1
    ...
    private static final String THE_CAULDRON_OF_ETERNITY = "The Cauldron of Eternity"; // Whenever a creature you control dies, put it on the bottom of its owner’s library.
    ...
    private static final String USING_PROTOTYPE = " using Prototype";
    ...
    public void setupTestMutateSymbiote(boolean withToken, boolean mutateUnder) {
        ...
        addCard(Zone.BATTLEFIELD, playerA, INTANGIBLE_VIRTUE);
        addCard(Zone.HAND, playerA, DREAMTAIL_HERON);
        addCard(Zone.HAND, playerA, LEAD_ASTRAY);
        if (withToken) {
            addCard(Zone.HAND, playerA, ADVENT_OF_THE_WURM);
        ...
        assertAbility(playerA, creature, withToken ? TrampleAbility.getInstance() : HasteAbility.getInstance(), true);
        assertAbility(playerA, creature, FlyingAbility.getInstance(), true);
        assertAbility(playerA, creature, VigilanceAbility.getInstance(), withToken && mutateUnder);
        if (withToken) {
    ...
    /**
     * Mutate onto a permanent that becomes a creature
     */
    public void setupTestReptilianReflection(boolean mutateUnder) {
        setupLands(playerA);

        addCard(Zone.HAND, playerA, BACKSLIDE); // cycling U
        addCard(Zone.HAND, playerA, DREAMTAIL_HERON);
        addCard(Zone.BATTLEFIELD, playerA, REPTILIAN_REFLECTION);

        activateAbility(1, PhaseStep.PRECOMBAT_MAIN, playerA, "Cycling");
        waitStackResolved(1, PhaseStep.PRECOMBAT_MAIN);
        setChoice(playerA, true); // become creature

        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerA, DREAMTAIL_HERON + USING_MUTATE, REPTILIAN_REFLECTION);

        setChoice(playerA, mutateUnder);

        checkAbility("RR", 1, PhaseStep.BEGIN_COMBAT, playerA, mutateUnder ? REPTILIAN_REFLECTION : DREAMTAIL_HERON, FlyingAbility.class, true);
        checkAbility("RR", 1, PhaseStep.BEGIN_COMBAT, playerA, mutateUnder ? REPTILIAN_REFLECTION : DREAMTAIL_HERON, TrampleAbility.class, true);
        checkPT("RR", 1, PhaseStep.BEGIN_COMBAT, playerA, mutateUnder ? REPTILIAN_REFLECTION : DREAMTAIL_HERON, 5, 4);
        ...
        assertAbility(playerA, mutateUnder ? REPTILIAN_REFLECTION : DREAMTAIL_HERON, FlyingAbility.getInstance(), true);
        assertAbility(playerA, mutateUnder ? REPTILIAN_REFLECTION : DREAMTAIL_HERON, TrampleAbility.getInstance(), false);
        assertType(mutateUnder ? REPTILIAN_REFLECTION : DREAMTAIL_HERON, CardType.CREATURE, !mutateUnder);

    }
    @Test
    public void testMutateCardUnderReptilianReflection() {
        setupTestReptilianReflection(true);
    }
    @Test
    public void testMutateCardOverReptilianReflection() {
        setupTestReptilianReflection(false);
    }

    /**
     * Mutate onto a permanent that becomes a creature, then copy it
     */
    @Test
    public void testMutateCardOverReptilianReflectionCopy() {
        setupLands(playerA);
        setupLands(playerB);

        addCard(Zone.HAND, playerA, BACKSLIDE, 2); // cycling U
        addCard(Zone.HAND, playerA, DREAMTAIL_HERON);
        addCard(Zone.HAND, playerB, RITE_OF_REPLICATION);
        addCard(Zone.BATTLEFIELD, playerA, REPTILIAN_REFLECTION);
        ...
        setChoice(playerA, false); // mutate over

        activateAbility(2, PhaseStep.UPKEEP, playerA, "Cycling");
        setChoice(playerA, true); // become creature again

        checkAbility("RR", 1, PhaseStep.BEGIN_COMBAT, playerA,  DREAMTAIL_HERON, FlyingAbility.class, true);
        checkAbility("RR", 1, PhaseStep.BEGIN_COMBAT, playerA, DREAMTAIL_HERON, TrampleAbility.class, true);
        checkPT("RR", 1, PhaseStep.BEGIN_COMBAT, playerA, DREAMTAIL_HERON, 5, 4);

        castSpell(2, PhaseStep.PRECOMBAT_MAIN, playerB, RITE_OF_REPLICATION);
        setChoice(playerB, false); // no kicker
        addTarget(playerB, DREAMTAIL_HERON);

        setStopAt(2, PhaseStep.POSTCOMBAT_MAIN);
        setStrictChooseMode(true);
        execute();

        assertAbility(playerB, DREAMTAIL_HERON, FlyingAbility.getInstance(), true);
        assertAbility(playerB, DREAMTAIL_HERON, TrampleAbility.getInstance(), false);
        assertPowerToughness(playerB, DREAMTAIL_HERON, 3, 4);

    }

    /**
     * Test interaction with Prototype ability
     */
    public void setupTestMutatePrototype(boolean withPrototype, boolean mutateUnder) {
        setupLands(playerA);

        addCard(Zone.HAND, playerA, DREAMTAIL_HERON);
        addCard(Zone.HAND, playerA, BLITZ_AUTOMATON);

        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerA, BLITZ_AUTOMATON + (withPrototype ? USING_PROTOTYPE : ""));

        castSpell(1, PhaseStep.POSTCOMBAT_MAIN, playerA, DREAMTAIL_HERON + USING_MUTATE, BLITZ_AUTOMATON);
        setChoice(playerA, mutateUnder);
        ...
        assertHandCount(playerA, DREAMTAIL_HERON, 0);
        assertPermanentCount(playerA, BLITZ_AUTOMATON, mutateUnder ? 1 : 0);
        assertPermanentCount(playerA, DREAMTAIL_HERON, mutateUnder ? 0 : 1);
        String creature = mutateUnder ? BLITZ_AUTOMATON : DREAMTAIL_HERON;
        assertColor(playerA, creature, "U", !mutateUnder);
        if (withPrototype) {
            assertPowerToughness(playerA, creature, 3, mutateUnder ? 2 : 4);
            assertColor(playerA, creature, "R", mutateUnder);
        } else {
            assertPowerToughness(playerA, creature, mutateUnder ? 6 : 3, 4);
            assertColor(playerA, creature, "R", false);
        }
    }
    @Test
    public void testMutateCardUnderNonPrototypedCard() {
        setupTestMutatePrototype(false, true);
    }
    @Test
    public void testMutateCardOverNonPrototypedCard() {
        setupTestMutatePrototype(false, false);
    }
    @Test
    public void testMutateCardUnderPrototypedCard() {
        setupTestMutatePrototype(true, true);
    }
    @Test
    public void testMutateCardOverPrototypedCard() {
        setupTestMutatePrototype(true, false);
    }
    ...
    public void setupTestMutatePhasing(boolean withToken, boolean mutateUnder) {
        ...
        assertPermanentCount(playerA, creature, 0);
        assertPermanentCount(playerA, SHORT_SWORD, 0);
    ...
    public void setupTestMutateMorph(boolean mutateUnder) {
        ...
        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerA, CORAL_TRICKSTER + " using Morph");

        castSpell(1, PhaseStep.POSTCOMBAT_MAIN, playerA, DREAMTAIL_HERON + USING_MUTATE, EmptyNames.FACE_DOWN_CREATURE.getTestCommand());
        setChoice(playerA, mutateUnder);
        ...
        if (mutateUnder) {
            ...
            assertAbility(playerA, EmptyNames.FACE_DOWN_CREATURE.getTestCommand(), FlyingAbility.getInstance(), false);
    ...
    /**
     * Check that effect to return enchanted creature gets all mutated cards
     */
    public void setupTestMutateReturnEnchanted(boolean mutateUnder) {
        setupLands(playerA);
        setupLands(playerB);

        addCard(Zone.BATTLEFIELD, playerA, BANEHOUND);
        addCard(Zone.HAND, playerA, DREAMTAIL_HERON);
        addCard(Zone.HAND, playerB, FUNGAL_FORTITUDE);
        addCard(Zone.HAND, playerB, MURDER);

        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerB, FUNGAL_FORTITUDE, BANEHOUND);

        castSpell(1, PhaseStep.POSTCOMBAT_MAIN, playerA, DREAMTAIL_HERON + USING_MUTATE, BANEHOUND);
        setChoice(playerA, mutateUnder);
        String creature = mutateUnder ? BANEHOUND : DREAMTAIL_HERON;
        ...
        assertGraveyardCount(playerB, FUNGAL_FORTITUDE, 1);
        assertGraveyardCount(playerB, MURDER, 1);
        assertPermanentCount(playerA, BANEHOUND, 1);
        assertPermanentCount(playerA, DREAMTAIL_HERON, 1);
        assertTapped(BANEHOUND, true);
        assertTapped(DREAMTAIL_HERON, true);
    }
    @Test
    public void testMutateCardUnderReturnEnchanted() {
        setupTestMutateReturnEnchanted(true);
    }
    @Test
    public void testMutateCardOverReturnEnchanted() {
        setupTestMutateReturnEnchanted(false);
    }
    ...
    /**
     * Flip mutate test - not currently working
     */
    ...
    /**
     * Dies source return to hand mutate test
     */
    public void setupTestMutateDiesReturnToHand(boolean mutateUnder) {
        setupLands(playerA);
        setupLands(playerB);

        addCard(Zone.HAND, playerB, MURDER);

        addCard(Zone.BATTLEFIELD, playerA, MORTUS_STRIDER);
        addCard(Zone.HAND, playerA, BROKKOS_APEX_OF_FOREVER);

        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerA, BROKKOS_APEX_OF_FOREVER + USING_MUTATE, MORTUS_STRIDER);
        setChoice(playerA, mutateUnder);

        castSpell(1, PhaseStep.POSTCOMBAT_MAIN, playerB, MURDER, mutateUnder ? MORTUS_STRIDER : BROKKOS_APEX_OF_FOREVER);

        setStopAt(1, PhaseStep.END_TURN);
        setStrictChooseMode(true);
        execute();

        assertHandCount(playerA, MORTUS_STRIDER, 1);
        assertHandCount(playerA, BROKKOS_APEX_OF_FOREVER, 1);
    }
    @Test
    public void setupTestMutateDiesReturnToHandUnder() {
        setupTestMutateDiesReturnToHand(true);
    }
    @Test
    public void setupTestMutateDiesReturnToHandOver() {
        setupTestMutateDiesReturnToHand(false);
    }

    /**
     * Dies creature return to library mutate test
     */
    public void setupTestMutateDiesReturnToLibrary(boolean mutateUnder) {
        setupLands(playerA);
        setupLands(playerB);

        addCard(Zone.HAND, playerB, MURDER);
        addCard(Zone.BATTLEFIELD, playerA, THE_CAULDRON_OF_ETERNITY);
        addCard(Zone.BATTLEFIELD, playerA, BANEHOUND);
        addCard(Zone.HAND, playerA, BROKKOS_APEX_OF_FOREVER);

        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerA, BROKKOS_APEX_OF_FOREVER + USING_MUTATE, BANEHOUND);
        setChoice(playerA, mutateUnder);

        castSpell(1, PhaseStep.POSTCOMBAT_MAIN, playerB, MURDER, mutateUnder ? BANEHOUND : BROKKOS_APEX_OF_FOREVER);
        setChoice(playerA, BANEHOUND); // order to bottom of library

        setStopAt(1, PhaseStep.END_TURN);
        setStrictChooseMode(true);
        execute();

        assertLibraryCount(playerA, BANEHOUND, 1);
        assertLibraryCount(playerA, BROKKOS_APEX_OF_FOREVER, 1);
    }
    @Test
    public void setupTestMutateDiesReturnToLibraryUnder() {
        setupTestMutateDiesReturnToLibrary(true);
    }
    @Test
    public void setupTestMutateDiesReturnToLibraryOver() {
        setupTestMutateDiesReturnToLibrary(false);
    }
    ...
    public void setupTestMutateGodEternal(boolean mutateUnder) {
        ...
        setChoice(playerA, true);
        setChoice(playerA, DREAMTAIL_HERON); // order cards to library
    ...
    public void setupTestMutateMimicVat(boolean withToken, boolean mutateUnder, boolean mimicUnder) {
        ...
        boolean mimicTrigger = !(withToken && mutateUnder);
        if (mimicTrigger) {
            setChoice(playerA, true);
        }
        ...
        if (!withToken) {
            addTarget(playerA, mimicUnder && mutateUnder ? BANEHOUND : DREAMTAIL_HERON);
        }
        ...
        if (mimicTrigger) {
            ...
            assertPermanentCount(playerA, mimicUnder && mutateUnder ? BANEHOUND : DREAMTAIL_HERON, 1);
            assertAbility(playerA, mimicUnder && mutateUnder ? BANEHOUND : DREAMTAIL_HERON, HasteAbility.getInstance(), true);
            assertAbility(playerA, mimicUnder && mutateUnder ? BANEHOUND : DREAMTAIL_HERON, withToken ? TrampleAbility.getInstance() : mimicUnder && mutateUnder ? FlyingAbility.getInstance() : LifelinkAbility.getInstance(), false);
        } else {
            assertGraveyardCount(playerA, DREAMTAIL_HERON, 1);
            assertExileCount(playerA, DREAMTAIL_HERON, 0);
        }
