...
public class WitchKingOfAngmarTest extends CardTestPlayerBase {
    ...
    public void test_Sacrifice() {
        ...
        runCode("check ring bear", 2, PhaseStep.POSTCOMBAT_MAIN, playerA, (info, player, game) -> {
            Assertions.assertNotNull(playerA.getRingBearer(game));
            Assertions.assertEquals(witchKing, playerA.getRingBearer(game).getName());
            Assertions.assertNull(playerB.getRingBearer(game));
        });
