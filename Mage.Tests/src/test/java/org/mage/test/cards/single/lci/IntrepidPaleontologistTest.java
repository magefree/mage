...
public class IntrepidPaleontologistTest extends CardTestPlayerBase {
    ...
    // Make sure that you can cast an enemy card and that the dino has the finality counter
    @Test
    public void test_basic() {
        addCard(Zone.BATTLEFIELD, playerA, paleo);
        addCard(Zone.BATTLEFIELD, playerA, "Mountain", 2+3);
        addCard(Zone.GRAVEYARD, playerB, dino);
    ...
    public void test_double() {
        ...
        addCard(Zone.GRAVEYARD, playerB, dino);
        addCard(Zone.GRAVEYARD, playerB, "Scytheclaw Raptor");

        activateAbility(1, PhaseStep.PRECOMBAT_MAIN, playerA, "{2}: ",dino);
        waitStackResolved(1, PhaseStep.PRECOMBAT_MAIN);
        activateAbility(1, PhaseStep.PRECOMBAT_MAIN, playerA, "{2}: ","Scytheclaw Raptor");
        waitStackResolved(1, PhaseStep.PRECOMBAT_MAIN);
        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerA, dino, true);
        castSpell(1, PhaseStep.PRECOMBAT_MAIN, playerA, "Scytheclaw Raptor", true);
        ...
        assertPermanentCount(playerA, dino, 1);
        assertPermanentCount(playerA, "Scytheclaw Raptor", 1);
        assertCounterCount(dino, CounterType.FINALITY, 1);
        assertCounterCount("Scytheclaw Raptor", CounterType.FINALITY, 1);
    }
}
