...
//            }
    ...
    public boolean choose(Outcome outcome, Target target, UUID sourceId, Ability source, Game game, Map<String, Serializable> options) {
        ...
        while (canRespond()) {
            Set<UUID> targetIds = target.possibleTargets(sourceId, abilityControllerId, source, game);
            ...
            if (responseId != null) {
                ...
                if (target instanceof TargetPermanent) {
                    if (((TargetPermanent) target).canTarget(abilityControllerId, responseId, sourceId, source, game, false)) {
    ...
    public boolean chooseTarget(Outcome outcome, Target target, Ability source, Game game) {
        ...
        while (canRespond()) {
            Set<UUID> possibleTargets = target.possibleTargets(source == null ? null : source.getSourceId(), abilityControllerId, source, game);
        ...
        if (canRespond()) {
            ...
            if (isGameUnderControl()) { // TODO: remove to enable quick stop for controlling player
                ...
                if (game.getTurn().getStepType() == PhaseStep.DECLARE_ATTACKERS
                    ...
                    int possibleBlockersCount = game.getBattlefield().count(filter, null, playerId, null, game);
    ...
    public void selectAttackers(Game game, UUID attackingPlayerId) {
        ...
        while (canRespond()) {
            ...
            if (response.getString() != null
                ...
                if (attacker != null) {
                    if (filterCreatureForCombat.match(attacker, null, playerId, null, game)) {
                        selectDefender(game.getCombat().getDefenders(), attacker.getId(), game);
                    } else if (filterAttack.match(attacker, null, playerId, null, game) && game.getStack().isEmpty()) {
    ...
    public void selectBlockers(Ability source, Game game, UUID defendingPlayerId) {
        ...
        // stop skip on any/zero permanents available
        int possibleBlockersCount = game.getBattlefield().count(filter, null, playerId, source, game);
        boolean canStopOnAny = possibleBlockersCount != 0 && getControllingPlayersUserData(game).getUserSkipPrioritySteps().isStopOnDeclareBlockersWithAnyPermanents();
        boolean canStopOnZero = possibleBlockersCount == 0 && getControllingPlayersUserData(game).getUserSkipPrioritySteps().isStopOnDeclareBlockersWithZeroPermanents();
        ...
        while (canRespond()) {
            ...
            if (response.getBoolean() != null) {
                ...
                if (blocker != null) {
                    ...
                    if (filter.match(blocker, null, playerId, source, game)) {
                        selectCombatGroup(defendingPlayerId, blocker.getId(), game);
                    } else if (filterBlock.match(blocker, null, playerId, source, game)
    ...
    protected void selectCombatGroup(UUID defenderId, UUID blockerId, Game game) {
        ...
        if (!isExecutingMacro()) {
            ...
            Set<UUID> attackers = target.possibleTargets(null, playerId, null, game);
    ...
    public void assignDamage(int damage, java.util.List<UUID> targets, String singleTargetName, UUID attackerId, Ability source, Game game) {
        ...
        while (remainingDamage > 0 && canRespond()) {
            ...
            choose(Outcome.Damage, target, source.getSourceId(), source, game);
    ...
    public Mode chooseMode(Modes modes, Ability source, Game game) {
        ...
        if (modes.size() > 1) {
            ...
            for (Mode mode : modes.getAvailableModes(source, game)) {
                ...
                if (mode.getTargets().canChoose(source.getSourceId(), source.getControllerId(), source, game)) { // and needed targets have to be available
