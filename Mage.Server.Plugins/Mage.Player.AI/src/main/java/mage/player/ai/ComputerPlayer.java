...
public class ComputerPlayer extends PlayerImpl implements Player {
    ...
    public boolean choose(Outcome outcome, Target target, UUID sourceId, Ability source, Game game, Map<String, Serializable> options) {
        ...
        Set<UUID> possibleTargets = target.possibleTargets(sourceId, abilityControllerId, source, game);
        ...
        if (target.getOriginalTarget() instanceof TargetControlledPermanent) {
            ...
            targets = threats(abilityControllerId, sourceId, source, origTarget.getFilter(), game, target.getTargets());
            if (!outcome.isGood()) {
                Collections.reverse(targets);
            }
            for (Permanent permanent : targets) {
                if (origTarget.canTarget(abilityControllerId, permanent.getId(), sourceId, source, game, false) && !target.getTargets().contains(permanent.getId())) {
        ...
        if (target.getOriginalTarget() instanceof TargetPermanent) {
            ...
            if (outcome.isCanTargetAll()) {
                targets = threats(null, sourceId, source, filter, game, target.getTargets());
            } else {
                if (outcome.isGood()) {
                    targets = threats(abilityControllerId, sourceId, source, filter, game, target.getTargets());
                } else {
                    targets = threats(randomOpponentId, sourceId, source, filter, game, target.getTargets());
                }
        ...
        if (target.getOriginalTarget() instanceof TargetCardInHand
            ...
            for (UUID cardId : target.possibleTargets(sourceId, this.getId(), source, game)) {
        ...
        if (target.getOriginalTarget() instanceof TargetAnyTarget) {
            ...
            if (outcome.isGood()) {
                targets = threats(abilityControllerId, sourceId, source, ((FilterCreaturePlayerOrPlaneswalker) origTarget.getFilter()).getPermanentFilter(), game, target.getTargets());
            } else {
                targets = threats(randomOpponentId, sourceId, source, ((FilterCreaturePlayerOrPlaneswalker) origTarget.getFilter()).getPermanentFilter(), game, target.getTargets());
            }
            for (Permanent permanent : targets) {
        ...
        if (target.getOriginalTarget() instanceof TargetCreatureOrPlayer) {
            ...
            if (outcome.isGood()) {
                targets = threats(abilityControllerId, sourceId, source, ((FilterCreatureOrPlayer) origTarget.getFilter()).getCreatureFilter(), game, target.getTargets());
            } else {
                targets = threats(randomOpponentId, sourceId, source, ((FilterCreatureOrPlayer) origTarget.getFilter()).getCreatureFilter(), game, target.getTargets());
            }
            for (Permanent permanent : targets) {
        ...
        if (target.getOriginalTarget() instanceof TargetPermanentOrPlayer) {
            ...
            List<Permanent> ownedTargets = threats(abilityControllerId, sourceId, source, ((FilterPermanentOrPlayer) origTarget.getFilter()).getPermanentFilter(), game, target.getTargets());
            List<Permanent> opponentTargets = threats(randomOpponentId, sourceId, source, ((FilterPermanentOrPlayer) origTarget.getFilter()).getPermanentFilter(), game, target.getTargets());
            if (outcome.isGood()) {
        ...
        if (target.getOriginalTarget() instanceof TargetSource) {
            Set<UUID> targets;
            targets = target.possibleTargets(sourceId, abilityControllerId, source, game);
    ...
    public boolean chooseTarget(Outcome outcome, Target target, Ability source, Game game) {
        ...
        if (target.getOriginalTarget() instanceof TargetPermanentOrPlayer) {
            ...
            TargetPermanentOrPlayer origTarget = ((TargetPermanentOrPlayer) target.getOriginalTarget());
            if (outcome.isGood()) {
                targets = threats(abilityControllerId, source.getSourceId(), source, ((FilterPermanentOrPlayer) origTarget.getFilter()).getPermanentFilter(), game, target.getTargets());
            } else {
                targets = threats(randomOpponentId, source.getSourceId(), source, ((FilterPermanentOrPlayer) origTarget.getFilter()).getPermanentFilter(), game, target.getTargets());
            }
        ...
        if (target.getOriginalTarget() instanceof TargetPlayerOrPlaneswalker
            ...
            if (outcome.isGood()) {
                targets = threats(abilityControllerId, source.getSourceId(), source, ((FilterPermanentOrPlayer) target.getFilter()).getPermanentFilter(), game, target.getTargets());
            } else {
                targets = threats(randomOpponentId, source.getSourceId(), source, ((FilterPermanentOrPlayer) target.getFilter()).getPermanentFilter(), game, target.getTargets());
            }
        ...
        if (target.getOriginalTarget() instanceof TargetSpellOrPermanent) {
            ...
            if (outcome.isGood()) {
                targets = threats(abilityControllerId, source == null ? null : source.getSourceId(), source, origTarget.getPermanentFilter(), game, target.getTargets());
            } else {
                targets = threats(randomOpponentId, source == null ? null : source.getSourceId(), source, origTarget.getPermanentFilter(), game, target.getTargets());
            }
            if (targets.isEmpty() && required) {
                targets = threats(null, source == null ? null : source.getSourceId(), source, origTarget.getPermanentFilter(), game, target.getTargets());
                Collections.reverse(targets);
        ...
        if (target.getOriginalTarget() instanceof TargetCardInExile) {
            ...
            List<Card> cards = new ArrayList<>();
            for (UUID uuid : target.possibleTargets(source.getSourceId(), source.getControllerId(), source, game)) {
    ...
    public boolean chooseTargetAmount(Outcome outcome, TargetAmount target, Ability source, Game game) {
        ...
        if (outcome == Outcome.Damage) {
            ...
            // permanents kill
            for (UUID opponentId : opponents) {
                targets = threats(opponentId, sourceId, source, StaticFilters.FILTER_PERMANENT_CREATURE_OR_PLANESWALKER_A, game, target.getTargets());
        ...
        for (UUID opponentId : opponents) {
            if (outcome.isGood()) {
                targets = threats(getId(), sourceId, source, StaticFilters.FILTER_PERMANENT, game, target.getTargets());
            } else {
                targets = threats(opponentId, sourceId, source, StaticFilters.FILTER_PERMANENT, game, target.getTargets());
            }
        ...
        for (UUID opponentId : opponents) {
            if (!outcome.isGood()) {
                ...
                targets = threats(getId(), sourceId, source, StaticFilters.FILTER_PERMANENT, game, target.getTargets(), false);
            } else {
                targets = threats(opponentId, sourceId, source, StaticFilters.FILTER_PERMANENT, game, target.getTargets(), false);
            }
...
//        log.info("paying for " + unpaid.getText());
    ...
    public boolean chooseTarget(Outcome outcome, Cards cards, TargetCard target, Ability source, Game game) {
        ...
        List<Card> cardChoices = new ArrayList<>(cards.getCards(target.getFilter(), source != null ? source.getSourceId() : null, playerId, source, game));
    ...
    public Mode chooseMode(Modes modes, Ability source, Game game) {
        ...
        for (Mode mode : modes.getAvailableModes(source, game)) {
            ...
            if (mode.getTargets().canChoose(source.getSourceId(), source.getControllerId(), source, game)) { // and where targets are available
                return mode;
            }
        }
        return null;
    ...
    protected void findBestPermanentTargets(Outcome outcome, UUID abilityControllerId, UUID sourceId, Ability source, FilterPermanent filter, Game game, Target target,
        ...
        // search all
        for (Permanent permanent : game.getBattlefield().getActivePermanents(filter, abilityControllerId, sourceId, source, game)) {
    ...
    protected List<Permanent> threats(UUID playerId, UUID sourceId, Ability source, FilterPermanent filter, Game game, List<UUID> targets) {
        return threats(playerId, sourceId, source, filter, game, targets, true);
    }

    protected List<Permanent> threats(UUID playerId, UUID sourceId, Ability source, FilterPermanent filter, Game game, List<UUID> targets, boolean mostValueableGoFirst) {
        // most valuable/powerfull permanents goes at first
        List<Permanent> threats;
        if (playerId == null) {
            threats = game.getBattlefield().getActivePermanents(filter, this.getId(), sourceId, source, game); // all permanents within the range of the player
        } else {
            FilterPermanent filterCopy = filter.copy();
            filterCopy.add(new ControllerIdPredicate(playerId));
            threats = game.getBattlefield().getActivePermanents(filter, this.getId(), sourceId, source, game);
        }
