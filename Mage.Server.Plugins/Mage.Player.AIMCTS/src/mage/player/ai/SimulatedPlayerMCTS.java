...
//        logger.info("select blockers");
    ...
    protected boolean chooseRandomTarget(Target target, Ability source, Game game) {
        Set<UUID> possibleTargets = target.possibleTargets(source == null ? null : source.getSourceId(), playerId, source, game);
        if (possibleTargets.isEmpty()) {
            return false;
        }
    ...
    public boolean choose(Outcome outcome, Target target, UUID sourceId, Ability source, Game game, Map<String, Serializable> options) {
        ...
        return super.choose(outcome, target, sourceId, source, game, options);
    }
