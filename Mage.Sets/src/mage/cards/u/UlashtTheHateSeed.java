...
class UlashtTheHateSeedEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (permanent != null && player != null) {
            int amount = game.getBattlefield().count(filterRed, source.getSourceId(), source.getControllerId(), source, game);
            amount += game.getBattlefield().count(filterGreen, source.getSourceId(), source.getControllerId(), source, game);
            if (amount > 0) {
