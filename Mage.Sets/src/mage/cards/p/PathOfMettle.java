...
public final class PathOfMettle extends TransformingDoubleFacedCard {
    ...
    private static final FilterCreaturePermanent filterDamage
            = new FilterCreaturePermanent("creature that doesn't have first strike, double strike, vigilance, or haste");
    private static final FilterCreaturePermanent filterTransform
            = new FilterCreaturePermanent("creatures that have first strike, double strike, vigilance, and/or haste");
    ...
    public PathOfMettle(UUID ownerId, CardSetInfo setInfo) {
        ...
        this.getLeftHalfCard().addAbility(new EntersBattlefieldTriggeredAbility(
                new DamageAllEffect(1, "it", filterDamage)
        ));
        ...
        this.getLeftHalfCard().addAbility(new AttacksWithCreaturesTriggeredAbility(
                new TransformSourceEffect(), 2, filterTransform
        ).setTriggerPhrase(triggerPhrase));

        // Metzali, Tower of Triumph
        // <i>(Transforms from Path of Mettle.)</i>
        this.getRightHalfCard().addAbility(new SimpleStaticAbility(new InfoEffect("<i>(Transforms from Path of Mettle.)</i>")));
