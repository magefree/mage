...
class DruidOfPurificationEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (game.getBattlefield().count(filter, source.getControllerId(), source.getControllerId(), source, game) < 1) {
            return false;
        }
        ...
        for (UUID playerId : game.getState().getPlayersInRange(source.getControllerId(), game)) {
            ...
            target.clearChosen();
            player.choose(Outcome.DestroyPermanent, target, source.getSourceId(), source, game);
