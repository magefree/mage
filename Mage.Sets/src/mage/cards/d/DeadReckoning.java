...
class DeadReckoningEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (controller != null) {
            if (target1.canChoose(source.getSourceId(), source.getControllerId(), source, game)
                    && controller.choose(Outcome.Benefit, target1, source.getSourceId(), source, game)
                    && target2.canChoose(source.getSourceId(), source.getControllerId(), source, game)
                    && controller.choose(Outcome.Damage, target2, source.getSourceId(), source, game)) {
