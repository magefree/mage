...
public final class OrderOfTheMirror extends TransformingDoubleFacedCard {
    ...
    public OrderOfTheMirror(UUID ownerId, CardSetInfo setInfo) {
        super(
                ...
                new CardType[]{CardType.CREATURE}, new SubType[]{SubType.PHYREXIAN, SubType.KNIGHT}, "UW"
        );
        ...
        this.getLeftHalfCard().addAbility(new ActivateAsSorceryActivatedAbility(
                new TransformSourceEffect(), new ManaCostsImpl<>("{3}{W/P}")
        ));

        // Order of the Alabaster Host
        // Whenever Order of the Alabaster Host becomes blocked by a creature, that creature gets -1/-1 until end of turn.
        this.getRightHalfCard().addAbility(new BecomesBlockedByCreatureTriggeredAbility(
                new BoostTargetEffect(-1, -1)
                        .setText("that creature gets -1/-1 until end of turn"), false
        ));
    }
