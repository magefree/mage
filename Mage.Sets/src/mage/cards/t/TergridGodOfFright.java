...
class TergridsLaternEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (game.getBattlefield().count(StaticFilters.FILTER_CONTROLLED_PERMANENT_NON_LAND, source.getSourceId(), targetedPlayer.getId(), source, game) > 0) {
        ...
        switch (chosen) {
            case SACRIFICE_CHOICE:
                ...
                targetedPlayer.choose(Outcome.Sacrifice, target, source.getSourceId(), source, game);
