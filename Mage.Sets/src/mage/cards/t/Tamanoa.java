...
class TamanoaDealsDamageTriggeredAbility extends TriggeredAbilityImpl {
    ...
    public boolean checkTrigger(GameEvent event, Game game) {
        ...
        if (eventSourceObject != null && !eventSourceObject.isCreature(game) && isControlledBy(game.getControllerId(event.getSourceId()))) {
            this.getEffects().forEach(effect -> effect.setValue("damage", event.getAmount()));
            return true;
        }
        return false;
    }
