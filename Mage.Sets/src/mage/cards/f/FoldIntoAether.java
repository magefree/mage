    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (game.getStack().counter(targetId, source, game)) {
            ...
            if (spellController != null
                    && target.canChoose(source.getSourceId(), spellController.getId(), source, game)
                    && spellController.chooseUse(Outcome.Neutral, "Put a creature card from your hand in play?", source, game)
                    && spellController.choose(Outcome.PutCreatureInPlay, target, source.getSourceId(), source, game)) {
