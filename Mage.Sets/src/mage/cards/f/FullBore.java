...
class FullBoreEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (permanent == null
                || !Optional
                .ofNullable(game)
                .map(Game::getPermanentCostsTags)
                .map(map -> map.getOrDefault(new MageObjectReference(permanent, game, -1), null))
                .filter(map -> map.containsKey(WarpAbility.WARP_ACTIVATION_VALUE_KEY))
                .isPresent()) {
            return false;
        }
