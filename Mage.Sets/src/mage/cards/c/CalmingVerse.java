...
class CalmingVerseEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        for (Permanent permanent : game.getBattlefield().getActivePermanents(opponentEnchantmentsFilter, source.getControllerId(), source.getSourceId(), source, game)) {
        ...
        if (controller != null) {
            ...
            if (game.getState().getBattlefield().countAll(untappedLandFilter, controller.getId(), game) > 0) {
                for (Permanent permanent : game.getBattlefield().getActivePermanents(controlledEnchantmentsFilter, source.getControllerId(), source.getSourceId(), source, game)) {
                    permanent.destroy(source, game, false);
                }
            }
