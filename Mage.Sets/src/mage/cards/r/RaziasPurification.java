...
class RaziasPurificationEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        for (UUID playerId : game.getState().getPlayersInRange(source.getControllerId(), game)) {
            ...
            if (player != null && target1.canChoose(source.getSourceId(), player.getId(), source, game)) {
                int chosenPermanents = 0;
                while (player.canRespond() && !target1.isChosen() && target1.canChoose(source.getSourceId(), player.getId(), source, game) && chosenPermanents < 3) {
