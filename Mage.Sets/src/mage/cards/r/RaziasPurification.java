...
class RaziasPurificationEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        for (UUID playerId : game.getState().getPlayersInRange(source.getControllerId(), game)) {
            ...
            if (player != null && target1.hasPossibleChoices(player.getId(), game)) {
                int chosenPermanents = 0;
                while (player.canRespond() && !target1.isChosen() && target1.hasPossibleChoices(player.getId(), game) && chosenPermanents < 3) {
