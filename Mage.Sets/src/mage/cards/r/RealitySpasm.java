...
class RealitySpasmTapEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (target.canChoose(source.getSourceId(), source.getControllerId(), source, game) && target.choose(Outcome.Tap, source.getControllerId(), source.getSourceId(), source, game)) {
...
class RealitySpasmUntapEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        numberToTap = Math.min(game.getBattlefield().getAllActivePermanents().size(), numberToTap);
        TargetPermanent target = new TargetPermanent(numberToTap, filter);
        if (target.canChoose(source.getSourceId(), source.getControllerId(), source, game) && target.choose(Outcome.Untap, source.getControllerId(), source.getSourceId(), source, game)) {
            if (!target.getTargets().isEmpty()) {
                List<UUID> targets = target.getTargets();
                for (UUID targetId : targets) {
                    Permanent permanent = game.getPermanent(targetId);
