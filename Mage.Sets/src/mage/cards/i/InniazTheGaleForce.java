...
class InniazTheGaleForceEffect extends OneShotEffect {
    ...
    private static final class PlayerPair {
        ...
        private void chooseTargets(Player controller, Game game, Ability source) {
            if (game.getBattlefield().count(this.filter, source.getSourceId(), source.getControllerId(), source, game) > 0) {
                controller.choose(Outcome.Neutral, this.target, source.getSourceId(), source, game);
            }
        }
