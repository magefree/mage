...
class ViashinoBeyEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (controller != null) {
            game.getBattlefield().getAllActivePermanents(CardType.CREATURE, game).stream().filter((permanent) -> (filter.match(permanent, source.getSourceId(), source.getControllerId(), source, game))).forEachOrdered((permanent) -> {
                if (game.getOpponents(controller.getId()).size() > 1) {
                    controller.choose(outcome.Benefit, targetDefender, source.getSourceId(), source, game);
                } else {
