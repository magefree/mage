...
public final class VengefulPharaoh extends CardImpl {
    ...
    public VengefulPharaoh(UUID ownerId, CardSetInfo setInfo) {
        ...
        this.addAbility(new ConditionalInterveningIfBatchTriggeredAbility<>(
                new VengefulPharaohTriggeredAbility(), SourceInGraveyardCondition.instance,
                "Whenever combat damage is dealt to you or a planeswalker you control, "
                        + "if {this} is in your graveyard, destroy target attacking creature, "
                        + "then put {this} on top of your library"
        ));
    }
...
class VengefulPharaohTriggeredAbility extends TriggeredAbilityImpl implements BatchTriggeredAbility<DamagedEvent> {
    ...
    public VengefulPharaohTriggeredAbility() {
        ...
        this.addEffect(new PutOnLibrarySourceEffect(true));
    }
            ...
            case DAMAGED_BATCH_FOR_ONE_PERMANENT:
                return ((DamagedBatchForOnePermanentEvent) event)
                        ...
                        .filter(e -> {
                            ...
                            Permanent permanent = game.getPermanentOrLKIBattlefield(e.getTargetId());
