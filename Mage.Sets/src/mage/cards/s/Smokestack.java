    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (activePlayer != null && sourcePermanent != null) {
            ...
            if (count > 0) {
                ...
                if (target.canChoose(source.getSourceId(), activePlayer.getId(), source, game)) {
                    while (!target.isChosen() && target.canChoose(source.getSourceId(), activePlayer.getId(), source, game) && activePlayer.canRespond()) {
                        activePlayer.choose(Outcome.Sacrifice, target, source.getSourceId(), source, game);
                    }
