...
class StatecraftPreventionEffect extends PreventionEffectImpl {
    ...
    public boolean applies(GameEvent event, Ability source, Game game) {
        if (super.applies(event, source, game)) {
            ...
            if (sourcePermanent != null && filter.match(sourcePermanent, source.getSourceId(), source.getControllerId(), source, game)) {
                return true;
            }
            Permanent targetPermanent = game.getPermanent(event.getTargetId());
            if (targetPermanent != null && filter.match(targetPermanent, source.getSourceId(), source.getControllerId(), source, game)) {
                return true;
            }
        }
        return false;
