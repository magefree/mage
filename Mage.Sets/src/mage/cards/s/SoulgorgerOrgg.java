...
class SoulgorgerOrggLoseLifeEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (player != null) {
            ...
            game.getState().setValue(source.getSourceId().toString() + source.getControllerId().toString() + "_lifeValue", lifeValue);
            if (lifeValue > 0) {
...
class SoulgorgerOrggGainLifeEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        Player player = game.getPlayer(source.getControllerId());
        Object obj = game.getState().getValue(source.getSourceId().toString() + source.getControllerId().toString() + "_lifeValue");
