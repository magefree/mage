...
class HelmOfKaldraCondition implements Condition {
    ...
    public boolean apply(Game game, Ability source) {
        if (game.getBattlefield().count(HelmOfKaldra.filterHelm, source.getSourceId(), source.getControllerId(), source, game) < 1) {
            return false;
        }
        if (game.getBattlefield().count(HelmOfKaldra.filterSword, source.getSourceId(), source.getControllerId(), source, game) < 1) {
            return false;
        }
        return game.getBattlefield().count(HelmOfKaldra.filterShield, source.getSourceId(), source.getControllerId(), source, game) >= 1;
    }
