...
public final class HideousTaskmaster extends CardImpl {
    ...
    public HideousTaskmaster(UUID ownerId, CardSetInfo setInfo) {
        ...
        Ability ability = new CastSourceTriggeredAbility(new GainControlTargetEffect(Duration.EndOfTurn).setTargetPointer(new EachTargetPointer()));
        ability.setTargetAdjuster(new EachOpponentPermanentTargetsAdjuster());
        ability.addTarget(new TargetCreaturePermanent(0,1));
        ability.addEffect(new UntapTargetEffect().setTargetPointer(new EachTargetPointer()));
        ability.addEffect(new GainAbilityTargetEffect(HasteAbility.getInstance(), Duration.EndOfTurn).setTargetPointer(new EachTargetPointer()));
        ability.addEffect(new GainAbilityTargetEffect(TrampleAbility.getInstance(), Duration.EndOfTurn).setTargetPointer(new EachTargetPointer()));
        ability.addEffect(new GainAbilityTargetEffect(new AnnihilatorAbility(1), Duration.EndOfTurn).setTargetPointer(new EachTargetPointer()));
