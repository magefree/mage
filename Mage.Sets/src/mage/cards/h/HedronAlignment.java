...
class HedronAlignmentEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (controller != null && sourceObject != null) {
            ...
            if (controller.getHand().getCards(filterCard, source.getSourceId(), controller.getId(), source, game).isEmpty()) {
                return true;
            }
            if (controller.getGraveyard().getCards(filterCard, source.getSourceId(), controller.getId(), source, game).isEmpty()) {
                return true;
            }
            Cards cardsToCheck = new CardsImpl();
            cardsToCheck.addAll(game.getExile().getAllCards(game));
            if (cardsToCheck.count(filterCard, source.getSourceId(), controller.getId(), source, game) == 0) {
                return true;
            }
            controller.won(game);
            return true;
