    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (controller != null) {
            if (controller.chooseUse(outcome, "Destroy all enchantments? (otherwise all nonwhite enchantments are destroyed)", source, game)) {
                for (Permanent permanent : game.getBattlefield().getActivePermanents(new FilterEnchantmentPermanent(), controller.getId(), source.getSourceId(), source, game)) {
                    permanent.destroy(source, game, false);
                }
            } else {
                for (Permanent permanent : game.getBattlefield().getActivePermanents(filter, controller.getId(), source.getSourceId(), source, game)) {
                    permanent.destroy(source, game, false);
                }
            }
            return true;
