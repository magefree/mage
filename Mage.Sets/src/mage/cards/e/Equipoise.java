...
class EquipoiseEffect extends OneShotEffect {
    ...
    private void phaseOutCardType(Player controller, Player targetPlayer, CardType cardType, Ability source, Game game) {
        ...
        int numberController = game.getBattlefield().count(filter, source.getSourceId(), controller.getId(), source, game);
        int numberTargetPlayer = game.getBattlefield().count(filter, source.getSourceId(), targetPlayer.getId(), source, game);
