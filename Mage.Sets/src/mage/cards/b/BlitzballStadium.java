...
public final class BlitzballStadium extends CardImpl {
    ...
    public static final ValueHint hint = new ValueHint("Kind of counters on it", BlitzballStadiumValue.instance);
    ...
    public BlitzballStadium(UUID ownerId, CardSetInfo setInfo) {
        ...
        Ability supportAbility = new EntersBattlefieldTriggeredAbility(new AddCountersTargetEffect(CounterType.P1P1.createInstance())
        ...
        Ability ability = new SimpleActivatedAbility(
                new GainAbilityTargetEffect(
                        new DealsCombatDamageToAPlayerTriggeredAbility(
                                new DrawCardSourceControllerEffect(BlitzballStadiumValue.instance)
                                    .setText("draw a card for each kind of counter on it"),
                                false
                        ).addHint(hint)
                ).setText("Until end of turn, target creature gains \"Whenever this creature deals combat damage to a player, draw a card for each kind of counter on it\""),
