...
public final class BarrinsCodex extends CardImpl {
    ...
    public BarrinsCodex(UUID ownerId, CardSetInfo setInfo) {
        ...
        this.addAbility(new BeginningOfUpkeepTriggeredAbility(new AddCountersSourceEffect(CounterType.PAGE.createInstance()), true));

        //{4}, {T}, Sacrifice Barrin's Codex: Draw X cards, where X is the number of page counters on Barrin's Codex.
        Ability ability = new SimpleActivatedAbility(new DrawCardSourceControllerEffect(new CountersSourceCount(CounterType.PAGE)), new ManaCostsImpl<>("{4}"));
