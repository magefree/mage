...
class WarReportEffect extends OneShotEffect {
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (player != null) {
            int lifeToGain = game.getBattlefield().count(StaticFilters.FILTER_PERMANENT_CREATURE, source.getSourceId(), source.getControllerId(), source, game);
            lifeToGain += game.getBattlefield().count(new FilterArtifactPermanent(), source.getSourceId(), source.getControllerId(), source, game);
