...
class WeightOfConscienceTarget extends TargetControlledCreaturePermanent {
    ...
    public Set<UUID> possibleTargets(UUID sourceId, UUID sourceControllerId, Ability source, Game game) {
        ...
        if (this.getTargets().isEmpty()) {
            List<Permanent> permanentList = game.getBattlefield().getActivePermanents(filterUntapped, sourceControllerId, sourceId, source, game);
            ...
            for (Permanent permanent : permanentList) {
                ...
                if (game.getBattlefield().count(filter, sourceId, sourceControllerId, source, game) > 1) {
                    possibleTargets.add(permanent.getId());
                }
            }
            ...
            for (Permanent permanent : game.getBattlefield().getActivePermanents(filterUntapped, sourceControllerId, sourceId, source, game)) {
                if (permanent != null) {
                    possibleTargets.add(permanent.getId());
                }
            }
    ...
    public boolean canChoose(UUID sourceId, UUID sourceControllerId, Ability source, Game game) {
        for (Permanent permanent1 : game.getBattlefield().getActivePermanents(filterUntapped, sourceControllerId, sourceId, source, game)) {
            for (Permanent permanent2 : game.getBattlefield().getActivePermanents(filterUntapped, sourceControllerId, sourceId, source, game)) {
    ...
    public boolean canTarget(UUID id, Ability source, Game game) {
        ...
        if (this.getTargets().isEmpty()) {
            List<Permanent> permanentList = game.getBattlefield().getActivePermanents(filterUntapped, source.getControllerId(), source.getSourceId(), source, game);
            if (permanentList.size() < 2) {
                return false;
            }
            for (Permanent permanent : permanentList) {
                ...
                FilterPermanent filter = filterUntapped.copy();
                filter.add(new SharesCreatureTypePredicate(permanent));
                if (game.getBattlefield().count(filter, source.getSourceId(), source.getControllerId(), source, game) > 1) {
                    return true;
                }
            }
        } else {
