...
public class ScryfallImagesDownloadTest {
    ...
    public void test_Cards_DownloadLinks() throws Exception {
        ...
        Assertions.assertEquals("https://api.scryfall.com/cards/10e/268/en?format=image", urls.getBaseUrl());

        // various card
        urls = imageSource.generateCardUrl(new CardDownloadData("Grizzly Bears", "30A", "195", true, 1));
        Assertions.assertEquals("https://api.scryfall.com/cards/30a/195/en?format=image", urls.getBaseUrl());
        urls = imageSource.generateCardUrl(new CardDownloadData("Grizzly Bears", "30A", "492", true, 2));
        Assertions.assertEquals("https://api.scryfall.com/cards/30a/492/en?format=image", urls.getBaseUrl());

        // api link
        urls = imageSource.generateCardUrl(new CardDownloadData("Ajani, the Greathearted", "WAR", "184*", false, 0));
        Assertions.assertEquals("https://api.scryfall.com/cards/war/184★/en?format=image", urls.getBaseUrl());

        // direct api link
        urls = imageSource.generateCardUrl(new CardDownloadData("Command Tower", "REX", "26b", false, 0));
        Assertions.assertEquals("https://api.scryfall.com/cards/rex/26/en?format=image&face=back", urls.getBaseUrl());

        // the one ring
        Assertions.assertTrue(TheLordOfTheRingsTalesOfMiddleEarth.getInstance().getSetCardInfo()
                .stream()
                .filter(c -> c.getName().equals("The One Ring"))
                .anyMatch(c -> c.getCardNumber().equals("001")),
                "LTR must use The One Ring with 001 number, not 0"
        );
        urls = imageSource.generateCardUrl(new CardDownloadData("The One Ring", "LTR", "001", false, 0));
        Assertions.assertEquals("https://api.scryfall.com/cards/ltr/0/qya?format=image", urls.getBaseUrl());
        ...
        Assertions.assertEquals("https://api.scryfall.com/cards/10e/268/en?format=image&version=small", urls.getBaseUrl());

        // various card
        urls = imageSourceSmall.generateCardUrl(new CardDownloadData("Grizzly Bears", "30A", "195", true, 1));
        Assertions.assertEquals("https://api.scryfall.com/cards/30a/195/en?format=image&version=small", urls.getBaseUrl());
        urls = imageSourceSmall.generateCardUrl(new CardDownloadData("Grizzly Bears", "30A", "492", true, 2));
        Assertions.assertEquals("https://api.scryfall.com/cards/30a/492/en?format=image&version=small", urls.getBaseUrl());

        // api link
        urls = imageSourceSmall.generateCardUrl(new CardDownloadData("Ajani, the Greathearted", "WAR", "184*", false, 0));
        Assertions.assertEquals("https://api.scryfall.com/cards/war/184★/en?format=image&version=small", urls.getBaseUrl());

        // direct api link
        urls = imageSourceSmall.generateCardUrl(new CardDownloadData("Command Tower", "REX", "26b", false, 0));
        Assertions.assertEquals("https://api.scryfall.com/cards/rex/26/en?format=image&version=small&face=back", urls.getBaseUrl());
        ...
        Assertions.assertEquals("https://api.scryfall.com/cards/ltr/0/qya?format=image&version=small", urls.getBaseUrl());
    }
}
